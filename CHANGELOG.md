...
- 优化 read_excel() 数据类型推断，采样行数由 100 提升至 1000，提高类型判断准确性
- 修复 Excel 类型推断未跳过表头行，列名不再被当作 Utf8 污染类型检测
- 修复整数与小数混存列被误判为 Int64 导致读取时小数截断的问题，现正确提升为 Float64
- 混合文本与数值或日期的列统一回退为 Utf8，避免读取时丢失单元格内容

v2.12.0 - 2026-06-22

- 新增 SQL 导出 UPDATE 语句支持多选 WHERE 字段，生成的语句以 AND 连接多个条件
- WHERE 字段多选时自动从 SET 子句中排除，避免重复更新
- 新增重复 WHERE 字段与空 WHERE 字段的校验与错误提示
